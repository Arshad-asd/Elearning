      ...
      {adminInfo && adminInfo.role === 'admin' && (
        <aside className={`admin-sidebar ${isIconsOnly ? 'icons-only' : ''}`}>
          ...
          <ul>
            ...
            <NavLink to='/admin/category-management' className="active-link" style={{ textDecoration: 'none', color: 'black' }}>
            <li onClick={toggleSubCategories}>
              <TbCategory className={`sidebar-icon ${isIconsOnly ? 'hidden' : ''}`} />
              <span className={`menu-text ${isIconsOnly ? 'hidden' : ''}`}>Category</span>
            </li>
            </NavLink>
            {showSubCategories && (
              <ul className="sub-categories">
                <li style={{marginLeft:'50px',}}>
                  <NavLink to='/admin/subcategory' style={{ textDecoration: 'none', color: 'black' }}>
                    <span className={`menu-text ${isIconsOnly ? 'hidden' : ''}`}>SubCategory</span>
                  </NavLink>
                </li>
              </ul>
            )}
